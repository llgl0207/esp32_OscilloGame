...
static uint8_t active_game_id = 0;                          // 当前活跃游戏 ID (0=无, 1=坦克, 2=乒乓球)
static TankData remote_tank_data;                           // 远程坦克数据
static PongData remote_pong_data;                           // 远程乒乓球数据
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
    ...
    // 6. 游戏开始
        ...
        if (current_state == NET_CONNECTED || current_state == NET_IN_GAME) {
            if (memcmp(connected_peer_mac, msg->src_mac, 6) == 0) {
                ...
                active_game_id = game_request_id;
    ...
    // 7. 游戏数据
        ...
        if (current_state == NET_IN_GAME) {
            if (memcmp(connected_peer_mac, msg->src_mac, 6) == 0) {
                // 同时存储两种游戏数据（union 共享内存，各游戏读各自结构体）
                remote_tank_data = msg->payload.tank_data;
                remote_pong_data = msg->payload.pong_data;
            }
        }
    }
    // 8. 游戏结束
...
void Network_Manager::sendGameData(const PongData& data) {
    if (current_state != NET_IN_GAME) return;
    ...
    memcpy(msg.src_mac, my_mac, 6);
    strncpy(msg.name, my_name, 16);
    msg.payload.pong_data = data;

    esp_now_send(connected_peer_mac, (uint8_t *) &msg, sizeof(msg));
}

bool Network_Manager::getRemoteGameData(PongData* dataOut) {
    *dataOut = remote_pong_data;
    return true;
}

uint8_t Network_Manager::getActiveGameId() {
